...
const AddTask = ({ taskData = null, onSubmit, isEditing = false }) => {
  const { id } = useParams();
    ...
    if (isEditing && id && !taskData) {
      ...
      API.get(`/tasks/${id}`, {
        ...
        .then((res) => {
          ...
          setTags(task.tags || ["Daily Routine"]);
        })
        .catch(() => {
          alert.error("Failed to load task data.");
        })
        .finally(() => setLoading(false));
    } else if (taskData && isEditing) {
      setTitle(taskData.title || "");
      setDescription(taskData.description || "");
      setColor(taskData.color || "#22c55e");
      ...
      setTags(taskData.tags || ["Daily Routine"]);
    }
  }, [isEditing, id, token, taskData]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing && id) {
        await API.put(`/tasks/${id}`, taskPayload, {
          headers: {
  ...
  return (
    <div className="min-h-screen bg-white px-5">
      ...
      <div className="mb-6">
        <h3 className="font-semibold my-4">Status</h3>
        <div className="flex flex-wrap gap-3">
          {["pending", "in-progress", "completed"].map((statusOption) => (
            <button
              key={statusOption}
              onClick={() => setStatus(statusOption)}
              className={`px-5 py-2 rounded-lg capitalize transition-colors ${
                status === statusOption
                  ? "bg-gray-200"
      ...
      <div className="flex flex-col lg:flex-row lg:space-x-20 bg-gray-100 rounded-xl shadow-md p-4 md:p-6 my-10 space-y-6 lg:space-y-0">
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className={`relative w-9 h-4 rounded-full transition-colors ${
                  repeat.enabled ? "bg-gray-800" : "bg-gray-300"
                }`}
              >
                <div
              ...
              <div className="flex justify-evenly bg-gray-50 rounded-2xl">
                {["daily", "weekly", "monthly"].map((type) => (
                  <button
                    ...
                    className={`px-4 md:px-8 py-2 rounded-2xl text-sm font-medium capitalize transition-colors ${
                      repeat.type === type
              ...
              <div className="flex flex-wrap gap-2">
                {days.map((day) => (
                  <button
                    ...
                    className={`px-3 md:px-5 py-2 rounded-lg text-sm font-medium min-w-[40px] md:min-w-[45px] transition-colors ${
      ...
      <div className="fixed bottom-8 right-4 md:right-8">
        <button
          ...
          disabled={loading}
          className={`w-12 h-12 bg-gray-500 rounded-full flex items-center justify-center shadow-lg transition-colors ${
